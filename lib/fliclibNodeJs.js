...
var FlicRawClient = function (inetAddress, port) {
  ...
  let opts = { host: inetAddress, port: port }
  ...
  function onOpen(isReconnect) {
    if (me.isDestroyed) return

    me.onOpen(isReconnect)
  }
  ...
  function onClose(had_error) {
    ...
    // Auto-reconnect if closed. If forced closed by client/user,
    // then socket will be destroyed and will not reconnect

    socket.once('connect', () => onOpen(true))
    setTimeout(() => socket.connect(opts), 2000)
  }
  ...
  function onData(data) {
    ...
    currentPacketData =
      currentPacketData == null
        ? data
        : Buffer.concat([currentPacketData, data], currentPacketData.length + data.length)
  ...
  this.onOpen = function (isReconnect) {}
...
var FlicClient = function (host, port) {
  ...
  rawClient.onOpen = function (isReconnect) {
    // Reconnect any channels that already exist.
    // Useful for when the connection to the server is
    // lost and auto-reconnects.
    for (var connId in connectionChannels) {
      if (connectionChannels.hasOwnProperty(connId)) {
        connectionChannels[connId]._attach(rawClient)
      }
    }
    me.emit('ready', isReconnect)
  }
    ...
    switch (opcode) {
      ...
      case FlicEventOpcodes.ButtonSingleOrDoubleClickOrHold: {
        if (connectionChannels[event.connId]) {
          ...
          if (
            (opcode == FlicEventOpcodes.CreateConnectionChannel && event.error != 'NoError') ||
